...
object Plugin extends sbt.Plugin {
  ...
  object LicenseKeys {
    ...
    lazy val licenseTests = SettingKey[Boolean]("licenseTests", "Add license header to test files")
  }
  ...
  private def modifySources(sourceDir: File, licenseText: String,
    ...
    (sourceDir ** "*.java").get foreach { path =>
      val fileContents = IO.read(path)
  ...
  private def formatLicenseHeadersTask =
    (streams, scalaSource in Compile, scalaSource in Test, license in formatLicenseHeaders, removeExistingHeaderBlock in formatLicenseHeaders, licenseTests in formatLicenseHeaders) map {
      (out, compileSourceDir, testSourceDir, lic, removeHeader, licTests) =>
        {
          modifySources(compileSourceDir, lic, removeHeader, out.log)
          if (licTests) modifySources(testSourceDir, lic, removeHeader, out.log)
        }
    }
}
